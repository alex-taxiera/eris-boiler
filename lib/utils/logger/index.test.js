...
test.serial('success', (t) => {
  ...
  t.true(t.context.logSpy.calledOnceWithExactly(content, 'green'))
})

test.serial('warn', (t) => {
  const content = 'warn log...'
  logger.warn(content)
  t.true(t.context.logSpy.calledOnceWithExactly(content, 'yellow'))
})

test.serial('error', (t) => {
  const content = 'error log...'
  logger.error(content)
  t.true(t.context.logSpy.calledOnceWithExactly(content, 'red'))
})
