...
class RAMManager extends DatabaseManager {
   ...
   * @param   {any}    [object] Optional object to insert.
   * @returns {void}
   */
  _addStoreIfNeeded (type, object) {
    ...
    if (object) {
      this._store.get(type).set(object.id, object)
    }
  }

  _passingValue (record, query) {
    return Object.keys(query.conditions).every((condition) => {
  ...
  _filterRecord (record, query) {
    let passing = this._passingValue(record, query)
  ...
  async delete (object) {
    this._addStoreIfNeeded(object.type, object)
