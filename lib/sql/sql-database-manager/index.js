...
const SQLSetting = require('../sql-setting')
/**
...
class DatabaseManager {
  ...
  getDefaultStatus () {
    return this._qb.get({ table: 'statuses', columns: ['name', 'type'], where: { default: true } })
      .then((status) => status ? new Status(status.name, status.type) : null)
  }
  /**
   * Get data on a guild from the guild_settings table
   * @param  {String}   id           The ID of the guild.
   * @param  {String[]} settingNames The names of the settings to fetch.
   * @return {Object}                Guild settings mapped by name.
   */
  async getSettings (id, settingNames) {
    const settings = await this._qb.get({ table: 'guild_settings', where: { id } })
    return Object.keys(settings)
      .filter((name) => settingNames.includes(name))
      .reduce((store, name) => ({ ...store, [name]: new SQLSetting(settings[name]) }), {})
  }
  /**
  ...
  getStatuses () {
    return this._qb.select({ table: 'statuses', columns: ['name', 'type'] })
      .then((statuses) => statuses.map((status) => new Status(status.name, status.type)))
  }
  /**
  ...
  async setSettings (settings) {
    const id = settings[0]._guildId
    await this._qb.update({ table: 'guild_settings', data: settings, where: { id } })
  }
  ...
  async _createTables (tables) {
    for (const table of tables) {
      await this._qb.createTable(table)
        .then((success) => logger.success(`Table ${table.name} created.`))
        .catch((error) => {
          if (error.message === `Table with name ${table.name} already exists.`) {
            logger.warn(`Table ${table.name} exists...`)
          }
        })
    }
  }
