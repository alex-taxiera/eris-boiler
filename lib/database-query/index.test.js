...
const buildGenericDBM = (mockData = [ { id: 1 } ]) => {
  ...
  sinon.stub(dbm, 'get').callsFake(
    async (query) =>
      mockData.find((item) => item[query.getKey] === query.getValue)
  )
...
test('get/calls dbm to get an entry and sets getValue', async (t) => {
  const type = 'testing this'
  ...
  t.is(dbQuery.getValue, 2)
  t.deepEqual(res, mockData[1])
})

test('get/calls dbm to get an entry and sets getKey', async (t) => {
  const type = 'testing this'
  const key = 'name'
  const mockData = [ { id: 1, [key]: 'whee' }, { id: 2, [key]: 'whoo' } ]
  const dbQuery = buildDBQuery(type, mockData)

  const res = await dbQuery.get('whee', key)

  t.is(dbQuery.getKey, key)
  t.deepEqual(res, mockData[0])
})
