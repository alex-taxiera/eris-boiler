...
test('_loadFiles/handles errors', async (t) => {
  const client = buildClient()
  const path = join(__dirname, 'broken-files')
...
test('_addLoadables/adds from array', (t) => {
  ...
  t.deepEqual(client._commandsToLoad, arr.reverse())
})
...
test('_addLoadables/works with individual elements and arrays', (t) => {
  ...
  t.is(client, testClient)
  t.deepEqual(client._commandsToLoad, [ ...arr, ...rest ].reverse())
})
...
test('_addLoadables/works with a single argument', (t) => {
  ...
  t.is(client, testClient)
  t.deepEqual(client._commandsToLoad, arr)
})
...
test('_loadLoadables/throws on unknown type', (t) => {
  ...
  return t.throwsAsync(
    () => client._loadLoadables('haha', client._commandsToLoad),
    {
      instanceOf: Error,
      message: `Unknown type: haha`
    }
  )
})

test('_loadCommand/loads command and aliases', (t) => {
  const client = buildClient()
  const command = { name: 'testing', aliases: [ 'one', 'two' ] }
  client._loadCommand(command)

  t.is(client.commands.get(command.name), command)
  t.is(client.aliases.get(command.aliases[0]), command.name)
  t.is(client.aliases.get(command.aliases[1]), command.name)
})
