...
// WARNING: This stubs connect for the whole test suite, and should only be used in one test
...
const buildClient = (token = 'abc', options = {
  erisOptions: {
    autoreconnect: true
  }
}) => {
  const bot = new DataClient(token, options)
  bot.commands.set('settings', new Command({ name: 'settings' }))
...
test('connect/calls loaders and super.connect', async (t) => {
  ...
  sinon.stub(client, '_loadLoadables')
  await client.connect()
  t.true(Client.prototype.connect.calledOnce)
  t.is(client._loadLoadables.callCount, 4)
})

test('connect/throws when there is no token', async (t) => {
  const client = buildClient('', {})
  await t.throwsAsync(
    () => client.connect(),
    {
...
test('_setOwner/calls API and sets owner', async (t) => {
  ...
  t.is(client.owners.length, 1)
  t.true(client.owners.every((o) => o.id === owner.id))
})
...
test('addSettingCommands/passes array to _addLoadables', (t) => {
  const client = buildClient()
  const arr = [ 1, 2, 3 ]

  const addLoadableSpy = sinon.spy(client, '_addLoadables')

  client.addSettingCommands(...arr)

  t.true(
    addLoadableSpy.calledOnceWithExactly(arr, client._settingCommandsToLoad)
  )
})
...
test('_loadLoadables/short circuits with an empty store', async (t) => {
  const client = buildClient()
  const loadCommandSpy = sinon.spy(client, '_loadCommand')

  await client._loadLoadables('commands', [])

  t.true(loadCommandSpy.notCalled)
})
...
test('_loadLoadables/loads setting commands', async (t) => {
  const client = buildClient()
  const commands = [ { name: 'test', aliases: [ 'alias' ] } ]

  sinon.stub(client, '_resolveLoadables')
    .callsFake(() => Promise.resolve(commands))
  const loadCommandSpy = sinon.spy(client, '_loadSettingCommand')

  await client._loadLoadables('settings', client._commandsToLoad)
