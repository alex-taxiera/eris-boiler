...
class DataClient extends require('eris').Client {
  ...
  constructor (options = {}) {
     ...
     * @type {Orator}
     */
    this.ora = new Orator(options.oraOptions)
    /**
  ...
  async statusSet (status) {
    ...
    logger.log(`${status.activity} ${status.name}`, 'cyan')
  ...
  async _onReady () {
    ...
    if (this.toggles.get('rotateStatus').on && statuses.length > 1) {
      this.statusRotateStart()
    }
  ...
  _selectLoader (name) {
    switch (name) {
      ...
      default:
        logger.error(`no "${name}" directory!`)
    }
  }
  /**
