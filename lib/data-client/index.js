...
const ExtendedCollection = require('../utils/extended-collection')
// const PermissionLibrary = require('../../permission-lib')
// const EventLibrary = require('../../event-lib')
// const CommandLibrary = require('../../command-lib')
...
class DataClient extends Client {
  ...
  constructor (token, {
    ...
    this.ora = new Orator(
      ...
      { ...defaults.oratorOptions, ...oraOptions }
    )
    /**
    ...
    /**
     * @type {ExtendedCollection} The command map.
     */
    this.commands = new ExtendedCollection()

    this.permissions = new ExtendedCollection()
  ...
  async _loadFiles (path) {
    ...
    return files.reduce((ax, dx) => {
      if (dx.endsWith('.js') && !dx.endsWith('.test.js')) {
        try {
          const data = require(join(path, dx))
          if (!data.isIndex) {
            ax.push(data)
          }
        } catch (e) {
