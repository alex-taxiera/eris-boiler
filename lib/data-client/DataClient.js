...
class DataClient extends require('eris').Client {
  ...
  _getDirectories (sourceFolder) {
    ...
    return Object.keys(dirs).reduce((store, key) => Object.assign(store, {
      [key]: {
        permissions: join(__dirname, dirs[key], 'permissions'),
        commands: join(__dirname, dirs[key], 'commands'),
        events: join(__dirname, dirs[key], 'events'),
        settings: join(__dirname, dirs[key], 'settings'),
        toggles: join(__dirname, dirs[key], 'toggles')
      }
    }), {})
  }
  /**
