...
const buildOrator = (prefix = 'x', options) => new Orator(prefix, options)
...
test('tryMessageDelete/logs delete error', async (t) => {
  const orator = buildOrator()
  const message = buildMessage({ userId: me.id })
  ...
  const res = await orator.tryMessageDelete(me, message)

  t.is(res[res.length - 1], colors.red(`Failed to delete: ${error}`))
})

test('tryCreateMessage/has sendMessages', async (t) => {
  const orator = buildOrator()
  const channelPerms = new Map()
  channelPerms.set('sendMessages', true)
  const message = buildMessage({ channelPerms })
...
test('tryCreateMessage/has no permission', async (t) => {
  const orator = buildOrator()
  const message = buildMessage()
  sinon.spy(message.channel, 'createMessage')
  const contents = [ 1, 2 ]
...
test('tryCreateMessage/logs create error', async (t) => {
  const orator = buildOrator()
  const channelPerms = new Map()
  channelPerms.set('sendMessages', true)
  const message = buildMessage({ channelPerms })
  ...
  const res = await orator.tryCreateMessage(me, message.channel, ...contents)

  t.is(res[res.length - 1], colors.yellow(`Failed to send: ${error}`))
})
...
test('_checkSubCommand/finds a subcommand', (t) => {
  ...
  sinon.stub(bot, 'findCommand').callsFake(() => ({ command: subCommand }))

  const res = orator._checkSubCommand({ params, bot, command })
...
test('_checkSubCommand/does not find a subcommand', (t) => {
  ...
  const res = orator._checkSubCommand({ bot, command, params })
...
test('_cleanParams/splits params by space', (t) => {
  ...
  t.deepEqual(orator._cleanParams(content), content.toLowerCase().split(/\s+/))
})
...
test('_tryToExecute/recursion when subcommand is found', async (t) => {
  ...
  sinon.stub(orator, '_checkSubCommand')
    .callsFake(() => ({ params: [], command: subCommand }))
...
test('processMessage/command throws an error', async (t) => {
  ...
  t.is(res[res.length - 1], colors.red(`error processing message: ${error}`))
  t.true(orator._cleanParams.calledOnceWithExactly(content))
  t.true(
    orator._parseParamsForCommand.calledOnceWithExactly([ content ], msg, bot)
  )
...
test('processMessage/response throws an error', async (t) => {
  ...
  t.is(
    res[res.length - 1],
    colors.red(`error processing command response: ${error}`)
  )
  t.true(orator._cleanParams.calledOnceWithExactly(content))
  t.true(
    orator._parseParamsForCommand.calledOnceWithExactly([ content ], msg, bot)
...
test('_parseParamsForCommand/happy path', async (t) => {
  ...
  sinon.stub(bot.dbm, 'newQuery')
    .callsFake(() => ({ get: async () => ({ get: () => null }) }))
  const context = { test: 'stuff' }
  sinon.stub(bot, 'findCommand').callsFake(() => context)
  bot.user = me
  const cmd = 'whee'
  const params = [ orator.defaultPrefix + cmd ]
  ...
  t.deepEqual(res, { ...context, msg, params })
  t.true(
    bot.findCommand.calledOnceWithExactly(cmd)
  )
})
...
test('_processCommandResponse/deletes response', async (t) => {
  ...
  await orator._processCommandResponse({ bot, msg, command }, response)
  clock.tick(command.deleteResponseDelay)
