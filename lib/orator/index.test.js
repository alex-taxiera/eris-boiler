...
test.failing('tryMessageDelete/logs delete error', async (t) => {
  ...
  t.is(res[res.length - 1], chalk.red(`Failed to delete: ${error}`))
})

test('tryCreateMessage/has sendMessages', async (t) => {
  const orator = buildOrator()
  const channelPerms = new Map()
  for (const perm of orator._requiredSendPermissions) {
    channelPerms.set(perm, true)
  }
  const message = buildMessage({ channelPerms })
...
test.failing('tryCreateMessage/logs create error', async (t) => {
  ...
  t.is(res[res.length - 1], chalk.yellow(`Failed to send: ${error}`))
})
