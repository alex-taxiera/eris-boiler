...
const { findOrCreateManagedWebhook } = require('../../util/webhook')
...
class Orator {
  ...
  async _processCommandResponse (bot, context, response) {
    ...
    if (response) {
      ...
      if (response.webhook) {
        ...
        try {
          const hook = await findOrCreateManagedWebhook(
            msg.channel,
            bot.user.id,
            this.webhookName
          )

          newMessage = await bot.executeWebhook(hook.id, hook.token, {
            ...content,
            username,
            avatarURL,
            wait: true
          ...
          logger.warn('Missing Webhook Permissions')

          newMessage = await this._missingWebhookPerms(response, bot, msg)
        }
      } else {
        newMessage = await (
          response.dm
  ...
  async _missingWebhookPerms (response, bot, msg) {
    const c = 'Sorry, this command requires the "Manage Webhooks" permission!'

    await response.dm
      ? this.createDirectMessage(bot.user, msg, c, undefined)
      : this.replyToMessage(bot.user, msg, c, undefined)
  }
}
 ...
 * @property {string}  [webhookName='Webhook Deliverer']  Default webhook name.
 */
