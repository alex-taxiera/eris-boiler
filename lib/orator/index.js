...
class Orator {
  ...
  tryMessageDelete (me, msg) {
    ...
    if (permissions.has('MANAGE_MESSAGES') || msg.author.id === me.id) {
      return msg.delete()
  ...
  async processMessage (bot, msg) {
    ...
    if (!this._isGuild(msg) || this._isBotMessage(bot.user, msg)) {
      return
    }
    ...
    if (this._isMentioned(bot.user, msg)) {
      return this._sendHelp(prefix, msg, bot.user)
    }

    const params = this._cleanParams(prefix, msg.content)

    if (params[0].startsWith(prefix)) {
      params[0] = params[0].substring(prefix.length)
    } else if (params[0] === bot.user.id) {
      params.shift()
    } else if (params[0].startsWith(bot.user.id)) {
      params[0] = params[0].substring(bot.user.id.length)
    } else {
      return
    }

    const context = bot.findCommand(params.shift())
    if (context) {
      Object.assign(context, { msg, params })
  ...
  async _tryToExecute (context) {
    ...
    const subContext = this._checkSubCommand()
  ...
  async _processCommandResponse (context, response) {
    ...
    if (response) {
      ...
      if (newMessage && deleteResponse && !response.badCommand) {
        setTimeout(
