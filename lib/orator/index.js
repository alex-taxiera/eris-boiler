...
class Orator {
  ...
  constructor (defaultPrefix, oratorOptions = {}) {
     ...
     * @type {string}
     */
    this.defaultPrefix = oratorOptions.defaultPrefix || defaultPrefix
    // /**
    //  * @type {boolean}
    //  */
  ...
  set permissions (permissions) {
    if (permissions) {
      this._permissions = permissions.sort((a, b) => a.level - b.level)
    }
  }
  ...
  async _tryToExecute (context) {
    ...
    const isPermitted = this.hasPermission(context)

    if (!isPermitted) {
      return this._badCommand(msg, 'You do not have the required permissions.')
    }
  ...
  async hasPermission (context) {
    ...
    const criteria = this._permissions.filter(
      (permission) => permission.level > command.options.permission.level
    )
