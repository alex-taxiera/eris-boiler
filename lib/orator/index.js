...
class Orator {
  ...
  async processMessage (bot, msg) {
    ...
    if (!this._isGuild(msg) || !this._isCommandByUser(bot.user, msg, prefix)) {
      return
    }
    ...
    const params = msg.content.substring(prefix.length).split(/\s+/).map((param) => {
    ...
    if (command) {
      const permission = bot.permissions.get(command.permission)
      const context = {
        bot,
        msg,
        params
  ...
  async _tryToExecute (command, context, permission) {
    ...
    if (bot.permissionLevel(msg.member) < command.permission) {
  ...
  async _execute (command, context) {
    ...
    if (subCommands.length > 0) {
      ...
      if (sub) {
        params.shift()
        const permission = bot.permissions.get(sub.permission)
