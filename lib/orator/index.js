...
class Orator {
  ...
  async processMessage (bot, msg) {
    if (this._isBotMessage(bot.user, msg)) {
      return
    }
  ...
  async _tryToExecute (bot, context) {
    ...
    if (command.dmOnly) {
      if (msg.channel.guild) {
        return this._badCommand(msg, 'only allowed in a dm')
      }
    } else if (command.guildOnly && !msg.channel.guild) {
      return this._badCommand(msg, 'only allowed in a guild')
    }
    ...
    const isPermitted = await this.hasPermission(bot, context)
