...
class Orator {
  ...
  async _tryToExecute (bot, context) {
    const { command, msg, params } = context
    let resp = { context, response: null }

    if (params.length < command.parameters.length) {
      resp = this._badCommand(msg, 'insufficient parameters!')
    }

    const permissionStatus = await this.hasPermission(bot, context)

    if (!permissionStatus.ok) {
      resp = this._badCommand(
        msg,
    ...
    if (subContext) {
      resp = this._tryToExecute(bot, subContext)
    }

    const response = await command.run(bot, context)

    if (resp.response) resp.response = response
  ...
  async hasPermission (bot, context) {
    ...
    if (command.dmOnly) {
      if (msg.channel.guild) {
        response.message = 'only allowed in a dm'
      }
    } else if (command.guildOnly && !msg.channel.guild) {
      response.message = 'only allowed in a guild'
    }
    ...
    for (const criterion of criteria) {
      if (await criterion.run(bot, context)) {
        response.ok = true
        response.message = `user has permission level ${criterion.level}`
      }
    }
   ...
   * @private
   * @param   {Message} msg   The message that was a bad command invocation {@link https://abal.moe/Eris/docs/Message|(link)}.
