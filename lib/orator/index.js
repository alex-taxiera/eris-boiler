...
class Orator {
  ...
  async processMessage (bot, msg) {
    ...
    if (this._isCommandByUser(bot.user, msg, prefix)) {
      ...
      if (command) {
        const permission =
          bot.permissions.find((perm) => perm.level === command.permission)
        const context = {
          bot,
          msg,
          params
  ...
  async _execute (command, context) {
    ...
    if (subCommands.length > 0) {
      ...
      if (sub) {
        params.shift()
        const permission =
          bot.permissions.find((perm) => perm.level === sub.permission)
  ...
  async _processCommandResponse (command, context, response) {
    ...
    if (deleteInvoking) {
      msg.delete().catch((e) => logger.warn('cannot delete messages')) // FIXME: check if we can delete messages in this channel first
    }
    if (!response) {
      return
    }
