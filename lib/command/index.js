...
class ToggleCommand extends SettingCommand {
  ...
  constructor (data) {
    super({
      ...data,
      getValue: function (bot, { channel }) {
        return bot.dbm.newQuery('guild').get(channel.guild.id)
          .then((dbGuild) => {
            if (!dbGuild) {
              throw Error('fuck')
            }

            return dbGuild.get(this.setting).toString()
          })
      },
      run: function (bot, { channel }) {
        return bot.dbm.newQuery('guild').get(channel.guild.id)
          .then((dbGuild) => {
            ...
            dbGuild.set(this.setting, !dbGuild.get(this.setting))
 ...
 * @property {string}                    setting     The name of the setting to use with the dbm.
 */
/**
