...
import { ExtendedMap, MaybeArray } from '@hephaestus/utils'
...
export abstract class Anvil<
  A,
  T extends A & Loadable = A & Loadable
> extends ExtendedMap<string, T> {
  ...
  constructor(private readonly key: ConditionalKeys<T, string>) {
    super()
  }

  public forge(...loadables: Array<MaybeArray<T | string>>): this {
    this.toLoad = this.toLoad.concat(
      loadables.reduce<Array<T | string>>(
        (ax, dx) => ax.concat(Array.isArray(dx) ? dx : [dx]),
        []
      )
    )
  ...
  public async reload(
    ...
    for (const loadable of loadables) {
      ...
      const oldLoadableObject = this.find(
        (old) => old.filePath === loadable.filePath
      )
